...
function renderNavbar(activeTab) {
  const navContainer =
    ...
    document.querySelector(".div > .div4:last-of-type");
  if (!navContainer) {
  ...
  const activeIdx =
    tabIndices[activeTab] !== undefined ? tabIndices[activeTab] : 0;
  ...
  navContainer.innerHTML = `
    ...
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; z-index: 2;">
      ...
      <!-- 설정 탭 -->
      <div class="nav-tab nav-setting" style="flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        <img src="assets/nav_setting.png" alt="설정" style="width: 24px; height: 24px; object-fit: contain; filter: ${activeTab === "setting" ? "none" : "brightness(0) invert(1)"};" />
      </div>
    </div>
  `;
  ...
  document.querySelector(".nav-setting").addEventListener("click", () => {
