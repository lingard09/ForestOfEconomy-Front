...
function renderNavbar(activeTab) {
  ...
  navContainer.innerHTML = `
    ...
    <div class="item" style="position: absolute; top: 5px; left: ${activeIdx * tabWidth}px; width: ${tabWidth}px; height: 40px; display: flex; align-items: center; justify-content: center; transition: left 0.35s cubic-bezier(0.4, 0, 0.2, 1); pointer-events: none; z-index: 1;">
    ...
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; z-index: 2;">
      ...
      <div class="nav-tab nav-home" style="flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        <img src="assets/nav_home.png" alt="홈" style="width: 24px; height: 24px; object-fit: contain; filter: ${activeTab === 'home' ? 'none' : 'brightness(0) invert(1)'};" />
      </div>
      
      <!-- 뉴스/배틀 탭 -->
      <div class="nav-tab nav-news" style="flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        <img src="assets/nav_news.png" alt="뉴스" style="width: 24px; height: 24px; object-fit: contain; filter: ${activeTab === 'news' ? 'none' : 'brightness(0) invert(1)'};" />
      </div>
      
      <!-- 그룹 탭 -->
      <div class="nav-tab nav-group" style="flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        <img src="assets/nav_group.png" alt="그룹" style="width: 24px; height: 24px; object-fit: contain; filter: ${activeTab === 'group' ? 'none' : 'brightness(0) invert(1)'};" />
      </div>
      
      <!-- 프로필 탭 -->
      <div class="nav-tab nav-profile" style="flex: 1; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        <img src="assets/nav_profile.png" alt="프로필" style="width: 24px; height: 24px; object-fit: contain; filter: ${activeTab === 'profile' ? 'none' : 'brightness(0) invert(1)'};" />
      </div>
    </div>
  `;
  ...
  document.querySelector('.nav-profile').addEventListener('click', () => {
    window.location.href = 'today.html'; // 프로필/로비 이동
  });
}
